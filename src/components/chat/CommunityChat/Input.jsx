...
const Input = () => {
  ...
  const handleSend = async () => {
    ...
    if (file) {
      const storageRef = ref(storage,  `CommunityChats/${data.chatId}/${uuid()}`);
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
            ...
            await updateDoc(doc(firestore, "CommunityChats", data.chatId), {
              messages: arrayUnion({
                id: uuid(),
                text,
    ...
    try{
      await updateDoc(doc(firestore, "CommunityChat", data.chatId), {
        "lastMessage": text,
